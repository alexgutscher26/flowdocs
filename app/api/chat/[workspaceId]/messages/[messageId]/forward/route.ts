...
export async function POST(
  ...
  try {
    ...
    if (!targetChannelIds || !Array.isArray(targetChannelIds) || targetChannelIds.length === 0) {
      return NextResponse.json({ error: "Target channel IDs are required" }, { status: 400 });
    }
    ...
    if (targetChannels.length !== targetChannelIds.length) {
      return NextResponse.json({ error: "One or more target channels not found" }, { status: 404 });
    }
