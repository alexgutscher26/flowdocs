...
export class WikiConverter {
   ...
   *
   * This function generates a summary of the provided messages. If the option `useAI` is enabled, it attempts to use an AI-based method to summarize the thread. In case of an error during the AI summarization, it falls back to a basic summary generated by the `generateBasicSummary` method, which also takes the `analysis` parameter into account.
   *
   * @param messages - An array of ExtendedMessage objects to be summarized.
   * @param analysis - An object containing analysis data related to the thread.
   */
  ...
  private detectLanguages(content: string): string[] {
    ...
    const patterns = {
      ...
      html: /\b(html|head|body|div|span|class|id|href|src)\b/i,
      json: /\b(object|array|string|number|boolean|null)\b/i,
      xml: /\b(xml|root|node|attribute|value)\b/i,
      yaml: /\b(yaml|key|value|list|map)\b/i,
      markdown: /\b(markdown|header|footer|section|paragraph)\b/i,
      rust: /\b(rust|fn|struct|impl|trait)\b/i,
      go: /\b(go|func|struct|interface|package)\b/i,
      php: /\b(php|class|function|namespace|public|private|protected)\b/i,
      ruby: /\b(ruby|class|def|module|private|public|protected)\b/i,
      swift: /\b(swift|class|struct|enum|protocol|func)\b/i,
      kotlin: /\b(kotlin|class|data|interface|object|fun)\b/i,
      scala: /\b(scala|class|trait|object|def|val|var)\b/i,
      groovy: /\b(groovy|class|interface|trait|def|static|private|public)\b/i,
      perl: /\b(perl|package|sub|my|our|local|state)\b/i,
      erlang: /\b(erlang|module|function|record|case|of)\b/i,
      elixir: /\b(elixir|defmodule|def|defp|defmacro|defprotocol)\b/i,
      julia: /\b(julia|function|module|struct|const|let|global)\b/i,
      r: /\b(r|r|function|data|model|plot|summary)\b/i,
      nextjs: /\b(nextjs|react|next|node|js|typescript|javascript)\b/i,
      nodejs: /\b(nodejs|react|next|node|js|typescript|javascript)\b/i,
      react: /\b(react|next|node|js|typescript|javascript)\b/i,
    };
