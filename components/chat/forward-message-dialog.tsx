...
export function ForwardMessageDialog({
  ...
  const handleForward = async () => {
    ...
    try {
      const response = await fetch(`/api/chat/${workspaceId}/messages/${message.id}/forward`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        toast({
          ...
          description: `Forwarded to ${selectedChannelIds.length} channel${
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="max-h-[200px] space-y-2 overflow-y-auto rounded-md border p-3">
          {isLoading ? (
            ...
            <p className="text-muted-foreground py-4 text-center text-sm">No channels found</p>
          ) : (
