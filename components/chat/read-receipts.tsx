...
export function ReadReceipts({ readBy, totalMembers, currentUserId }: ReadReceiptsProps) {
            ...
            {otherReaders.length > 0 && otherReaders.length <= 3 && (
              <div className="flex -space-x-1">
                {otherReaders.slice(0, 3).map((reader) => (
                  <Avatar key={reader.userId} className="border-background h-4 w-4 border">
        ...
        <TooltipContent side="bottom" align="end">
          <div className="space-y-1 text-xs">
            ...
            {otherReaders.length > 5 && (
              <p className="text-muted-foreground">and {otherReaders.length - 5} more...</p>
            )}
          </div>
        </TooltipContent>
      </Tooltip>
