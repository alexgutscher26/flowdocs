...
export function ChannelSettingsDialog({
  ...
  const handleArchive = async () => {
    setArchiving(true);
    try {
      const response = await fetch(`/api/chat/${workspaceId}/channels/${channel.id}/archive`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete <strong>#{channel.name}</strong>? This action cannot
              be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
                              ...
                              {nonMembers.map((user) => (
                                ...
                                >
                                  ...
                                  <div className="flex-1">
                                    <p className="text-sm">{user.user?.name || user.user?.email}</p>
                                    <p className="text-muted-foreground text-xs">
                                      {user.user?.email}
                                    </p>
                                  </div>
