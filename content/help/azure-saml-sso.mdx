---
title: Configure HagenKit
updatedAt: 2025-05-09
summary: Connect Azure Active Directory to HagenKit so your team signs in with existing corporate credentials.
author: codehagen
categories:
  - analysis
slug: azure-saml-sso
...
<Note variant="success">
  SAML single sign-on is available on HagenKit Scale and Enterprise plans.
</Note>

HagenKit Scale customers can enforce single sign-on through Azure Active Directory using the SAML 2.0 protocol. The steps below walk through the Azure configuration and the fields HagenKit expects.
...
In the next screen, click on **Create your own application**. Give your application a **Name** (e.g. "HagenKit") and click **Create**.
...
<CopyBox title="Identifier (Entity ID)" copy="https://sso.socialforge.app" />

<CopyBox
  title="Reply URL (Assertion Consumer Service URL)"
  copy="https://api.socialforge.app/auth/saml/callback"
/>
...
Scroll down to the 3rd section on the page, **SAML Certificates**. Copy the **App Federation Metadata Url** value—HagenKit uses it to keep your certificate in sync.
...
## Step 5: Configure SAML SSO on HagenKit

In your HagenKit workspace, open **Settings → Security**.

<Image
  alt="SAML SSO section on the HagenKit dashboard"
...
Under **SAML Single Sign-On**, click **Configure**. This opens the HagenKit SSO modal:

1. Select **Azure AD** as the provider.
2. Paste the **App Federation Metadata URL** from Azure.
3. Save your changes.
...
<Note variant="warning">
  We highly recommend configuring SCIM directory sync before assigning users & groups to your
  project. This will ensure that your users are automatically added to your project when they sign
  in for the first time, as well as automatically removed when they are deactivated in Azure.
</Note>
...
Your assigned users should now receive an invitation email to join your HagenKit workspace.
...
<Note variant="warning">
  ...
  This means that it may take up to 40 minutes for your users to receive the invitation email and be
  able to join your HagenKit workspace.
</Note>

They will also be able to sign in to HagenKit using Azure AD SSO.
