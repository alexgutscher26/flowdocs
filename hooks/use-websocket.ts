...
import { WebSocketEvent, WebSocketMessagePayload, WebSocketTypingPayload, WebSocketPresencePayload } from "@/types/chat";
...
export function useWebSocket({ workspaceId, userId, enabled = true }: UseWebSocketOptions) {
    ...
    const onPresence = useCallback((callback: (payload: WebSocketPresencePayload) => void) => {
        if (socketRef.current) {
            const handleOnline = (payload: WebSocketPresencePayload) => callback({ ...payload, status: "online" });
            const handleOffline = (payload: WebSocketPresencePayload) => callback({ ...payload, status: "offline" });
