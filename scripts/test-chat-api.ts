...
async function testChannelAPI() {
  ...
  try {
    ...
    const createResponse = await fetch(`${BASE_URL}/api/chat/${TEST_WORKSPACE_ID}/channels`, {
      method: "POST",
      headers: {
    ...
    console.log("\n2. Listing channels...");
    const listResponse = await fetch(`${BASE_URL}/api/chat/${TEST_WORKSPACE_ID}/channels`);
